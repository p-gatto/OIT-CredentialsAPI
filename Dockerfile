...
WORKDIR /src
COPY ["Services/Credentials/CredentialsAPI/CredentialsAPI.csproj", "Services/Credentials/CredentialsAPI/"]
RUN dotnet restore "./Services/Credentials/CredentialsAPI/CredentialsAPI.csproj"
COPY . .
WORKDIR "/src/Services/Credentials/CredentialsAPI"
RUN dotnet build "./CredentialsAPI.csproj" -c $BUILD_CONFIGURATION -o /app/build

# Questa fase viene usata per pubblicare il progetto di servizio da copiare nella fase finale
FROM build AS publish
ARG BUILD_CONFIGURATION=Release
RUN dotnet publish "./CredentialsAPI.csproj" -c $BUILD_CONFIGURATION -o /app/publish /p:UseAppHost=false
...
ENTRYPOINT ["dotnet", "CredentialsAPI.dll"]